...
from My_cost_dia import cost_fun
...
def arbitrary_rotation_function( params, Nq ):
	wires = [i for i in range(Nq)]
	weights = params
...
# 	arbitrary_rotation_function(paramsSet, Nq)
# 	return  qml.state()
# 	# return qml.probs(wires= qID )

def my_quantum_circuit_function(Nq, rho_matrix, paramsSet ):
	arbitrary_density_matrix_circuit( Nq, rho_matrix)
	wires = [i for i in range(Nq)]
	weights = paramsSet
	qml.ArbitraryUnitary(weights, wires)
...
def measurement_circuit_fun(Nq, rho_matrix, paramsSet):
	dev = qml.device("default.mixed", wires=Nq)
	circuit_run = qml.QNode( my_quantum_circuit_function, dev, interface="torch" )
	rho = circuit_run(Nq, rho_matrix, paramsSet)	
	P = torch.real(rho.diag() )
	# print('rho', rho)
	# print(qml.draw(circuit_run, decimals=None)(Nq, rho_matrix,  paramsSet))
	# print("P", P)
	return  P, rho
...
if __name__ == '__main__':
	...
	P, rho_f = measurement_circuit_fun(Nq, rho_matrix, paramsSet)
